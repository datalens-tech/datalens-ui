...
function isGroupControlGlobal(itemData: DashTabItemGroupControlData): boolean {
    ...
    const isGroupSettingsActive = itemData.group.every(
        (selector) => selector.tabsScope === undefined,
    );

    if (isControlGlobal(groupTabsScope) && isGroupSettingsActive) {
        return true;
    }
...
export function getDetailedGlobalStatus(
    ...
    if (itemType === DashTabItemType.GroupControl) {
        ...
        const isGroupSettingsActive = groupData.group.every(
            (selector) => selector.tabsScope === undefined,
        );

        if (groupTabsScope === TABS_SCOPE_ALL && isGroupSettingsActive) {
            hasAllScope = true;
        } else if (
            Array.isArray(groupTabsScope) &&
            groupTabsScope.length > 1 &&
...
export function getStateForControlWithGlobalLogic({
    ...
    if (state.openedItemId) {
        const existingGlobalItem = tabData.globalItems?.find(
            (item) => item.id === state.openedItemId,
        );
        const wasGlobal = Boolean(existingGlobalItem);

        if (wasGlobal && isGlobal && existingGlobalItem) {
