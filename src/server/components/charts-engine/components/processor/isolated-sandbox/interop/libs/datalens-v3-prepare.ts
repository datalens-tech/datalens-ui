...
const datalensV3prepareAdapter: typeof datalensModule = {
    ...
    buildChartsConfig: (args) =>
        JSON.parse(__libsDatalensV3_buildChartsConfig(JSON.stringify(args))),
    buildGraph: (...args) => JSON.parse(__libsDatalensV3_buildGraph(JSON.stringify(args))),
    buildHighchartsConfig: (args) =>
        JSON.parse(__libsDatalensV3_buildHighchartsConfig(JSON.stringify(args))),
    buildD3Config: (args) => JSON.parse(__libsDatalensV3_buildD3Config(JSON.stringify(args))),
};
