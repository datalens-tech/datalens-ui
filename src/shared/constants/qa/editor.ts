export const enum EditorTemplatesQA {
    NoTemplate = 'editor-templates-no-template-item',
    Selector = 'editor-templates-selector-item',
    Graph = 'editor-templates-graph-item',
    Module = 'editor-templates-module-item',
    YMap = 'editor-templates-y-map-item',
    Metric = 'editor-templates-metric-item',
    Table = 'editor-templates-table-item',
    Markdown = 'editor-templates-markdown-item',
    Timeseries = 'editor-templates-timeseries-item',
    Markup = 'editor-templates-markup-item',
    D3 = 'editor-templates-d3-item',
    WhiteBox = 'editor-templates-white-box-item',
}
